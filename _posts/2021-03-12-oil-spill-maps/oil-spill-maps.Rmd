...
In this report we make the following exploratory maps of the oil spill events that took place in California during 2008:

(1) chloropleth map showing the number of inland oil spill events by county, and

(2) interactive map showing the location of oil spill events included in the data and the number of inlaind oils spill events by county.
...
raw_counties <- read_sf(here("data","CA_counties","CA_Counties_TIGER2016.shp")) %>% 
  clean_names()

raw_spills <- read_sf(here("data","Oil_Spill_Tracking","Oil_Spill_Incident_Tracking_%5Bds394%5D.shp")) %>% 
  clean_names()
...
```{r}
# ---- CLOROPLETH MAP BY # OF INLAND SPILLS BY COUNTY
